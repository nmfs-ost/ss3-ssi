...
//    setJoinThree(10);
    ...
    connect(this, SIGNAL(autogenChanged(int)), this, SLOT(changeAutogen(int)));
...
void DialogSelexEquationView::cubicSpline(float scale) {
    ...
    if (numNodes >= 3) {
        ...
        selex->autogenParameters(flag);
        setAutogenComboBox(flag);
        ...
        if (parameters->rowCount() == num) {
            ...
            if (intScale == 2)
            {
                parameterView->setSliderRange(index, 1, bins.count());
            ...
            parameterView->setSliderRange(index, 0, 12);
            ...
            for (int i = 0; i < numNodes; i++) {
                parameterView->setSliderRange(index + i, bins.first(), bins.last());
...
void DialogSelexEquationView::updateCubicSpline(float scale) {
    ...
    {
        size = static_cast<unsigned>(special);
        ...
        start = intScale + 1;
        setup = getAutogenComboBox(); //static_cast<int>(parameterView->getInput(intScale) + .5);
        ...
        if (okay) {
            ...
            if (pointListIsOrdered(firstPoints))
            {
                ...
                // set selex to exp of spline values
                for (i = 0; i < firstPoints.count(); i++) {
                    temp2 = firstPoints.at(i).y();
                    secndPoints.append(QPointF(firstPoints.at(i)));
                    secndPoints[i].setY(exp(temp2 - temp1));
                }
                // set to 0 before first node for setup = 10, 11, and 12
                if (flag > 2 && ptSeries->points().at(0).x() > 0)
                {
                    i = 0;
                    while (secndPoints.at(i).x() < ptSeries->points().at(0).x())
                        secndPoints[i++].setY(0);
                }
...
void DialogSelexEquationView::changeAutogen(int value)
{
    flag = getAutogenComboBox();
    selex->autogenParameters(flag);
    if (equationNum == 27)
    {
        parameterView->setSliderValue(0, flag);
    }
    else if (equationNum == 42)
    {
        parameterView->setSliderValue(2, flag);
    }
}
