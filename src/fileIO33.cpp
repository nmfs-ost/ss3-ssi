...
/*            temp_float = d_file->next_value("equ_catch_se").toFloat();
            ...
            flt->setNumSeasons(data->num_seasons());
...
/*        if (num > 4) num = 5;
        ...
        temp_int = c_file->next_value().toInt(); // recruitment dist area
        pop->SR()->setDistribArea(temp_int);
        if (pop->SR()->getDistribMethod() == 4)
        {
            num = 1;
...
/*                for (int j = 0; j < num_vals; j++)
                {
                    ...
                    gp->setNatMParam(j + num_vals, datalist);
                }*/
        ...
        pop->SR()->rec_dev_phase = c_file->next_value().toInt();
        pop->SR()->advanced_opts = (c_file->next_value().compare("0") != 0);

        if (pop->SR()->advanced_opts)
        {
            pop->SR()->rec_dev_early_start = c_file->next_value().toInt();
            pop->SR()->rec_dev_early_phase = c_file->next_value().toInt();
            pop->SR()->fcast_rec_phase = c_file->next_value().toInt();
            pop->SR()->fcast_lambda = c_file->next_value().toFloat();
            pop->SR()->nobias_last_early_yr = c_file->next_value().toInt();
            pop->SR()->fullbias_first_yr = c_file->next_value().toInt();
            pop->SR()->fullbias_last_yr = c_file->next_value().toInt();
            pop->SR()->nobias_first_recent_yr = c_file->next_value().toInt();
            pop->SR()->max_bias_adjust = c_file->next_value().toFloat();
            pop->SR()->rec_cycles = c_file->next_value().toInt();
            pop->SR()->rec_dev_min = c_file->next_value().toInt();
            pop->SR()->rec_dev_max = c_file->next_value().toInt();
            pop->SR()->num_rec_dev = c_file->next_value().toInt();
...
int write33_controlFile(ss_file *c_file, ss_model *data)
{
    ...
    if(c_file->open(QIODevice::WriteOnly))
    {
        ...
        line = QString(QString("%1 # timevarying parameter autogenerate (0) or read (1)").arg (
                           QString::number(temp_int)));
        chars += c_file->writeline(line);
        chars += c_file->writeline("#");
...
//        chars += c_file->writeline (line);
        ...
        chars += c_file->writeline(line);
        line = QString("#_LO HI INIT PRIOR PR_type SD PHASE env_var&link dev_link dev_minyr dev_maxyr dev_phase Block Block_Fxn" );
        chars += c_file->writeline(line);
        num = pop->Grow()->getNum_patterns();
        for (i = 0; i < num; i++)
        {
...
/*        // deviation error params
        ...
        {
            line = QString(QString("#_Cond -4 #_MGparm_Dev_Phase" ));
            chars += c_file->writeline(line);
        }
        chars += c_file->writeline("#");*/
        ...
        chars += c_file->writeline(line);
        line = QString(QString ("%1 # SR_BH_steep" ).arg(pop->SR()->full_parameters->getRowText(1)));
        chars += c_file->writeline(line);
        if (pop->SR()->method == 5 ||
                pop->SR()->method == 7 ||
                pop->SR()->method == 8)
        ...
        chars += c_file->writeline(line);
        line = QString(QString ("%1 # SR_notused" ).arg(pop->SR()->full_parameters->getRowText(3)));
        chars += c_file->writeline(line);
...
/*        line = QString(QString ("%1 #_SR_env_link" ).arg(
        ...
        for (i = 0; i < data->num_fisheries(); i++)
            if (data->getFleet(i)->catch_equil() > 0.0)
                temp_int++;
        line = QString(QString("#_initial_F_parms; count = %1" ).arg (QString::number(temp_int)));
        chars += c_file->writeline(line);
        line = QString("#_LO HI INIT PRIOR PR_type SD PHASE" );
        chars += c_file->writeline(line);
        for (i = 0; i < data->num_fisheries(); i++)
...
/*        line = QString(QString ("%1 #_env/block/dev_adjust_method for selex parms (1=standard; 2=logistic transform keeps in base parm bounds; 3=standard w/ no bound check)").arg(
        ...
        line = QString ("#");
        chars += c_file->writeline(line);
        line = QString ("#_LO HI INIT PRIOR PR_type SD PHASE env-var use_dev dev_minyr dev_maxyr dev_stddev Block Block_Fxn # parm_name");
        chars += c_file->writeline(line);
        for (int i = 0; i < data->num_fleets(); i++)
        {
            selectivity *slx = data->getFleet(i)->getSizeSelectivity();
...
/*        line = QString (" #_custom_sel-env_setup (0/1) " );
        ...
        // Custom Block Setup
        line = QString (" #_custom_setup_for_sel_blocks&trends (0/1) ");
        num_vals = 0;
        for (int i = 0; i < data->num_fleets(); i++)
        {
...
/*        line = QString ("#_Cond -4 # placeholder for selparm_Dev_Phase" );
        ...
        chars += c_file->writeline(line);
        line = QString(QString("#_      LO        HI      INIT     PRIOR   PR_type        SD   PHASE"));
        chars += c_file->writeline(line);
        for (i = 0; i < data->num_fleets(); i++)
        { // size selectivity
            selectivity *slx = data->getFleet(i)->getSizeSelectivity();
